...
def _get_ngrams(n, text):
  ...
  for i in range(max_index_ngram_start + 1):
    x2ms_adapter.tensor_api.add(ngram_set, tuple(text[i:i + n]))
  return ngram_set


def _split_into_words(sentences):
  """Splits multiple sentences into words and flattens the result"""
  return list(itertools.chain(*[x2ms_adapter.tensor_api.split(_, " ") for _ in sentences]))
...
def rouge(hypotheses, references):
  ...
  rouge_1_f, rouge_1_p, rouge_1_r = map(x2ms_np.mean, zip(*rouge_1))

  # Calculate ROUGE-2 F1, precision, recall scores
  rouge_2 = [
      rouge_n([hyp], [ref], 2) for hyp, ref in zip(hypotheses, references)
  ]
  rouge_2_f, rouge_2_p, rouge_2_r = map(x2ms_np.mean, zip(*rouge_2))
  ...
  rouge_l_f, rouge_l_p, rouge_l_r = map(x2ms_np.mean, zip(*rouge_l))
