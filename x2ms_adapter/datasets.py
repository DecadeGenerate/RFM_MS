        ...
        for item in batch:
            ...
            if isinstance(item, dict):
                ...
                if _is_string_tensor(item):
                    cov_item = convert_to_utf8(item.asnumpy().tolist())
                    new_item = mindspore.dataset.text.to_str(np.array(cov_item)).tolist()
