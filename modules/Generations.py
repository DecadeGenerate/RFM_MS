...
def sample(model, data, vocab2id, max_len=20, encode_outputs=None, init_decoder_states=None):
    ...
    batch_size = x2ms_adapter.tensor_api.x2ms_size(data['id'], 0)
    ...
    indices = list()
    end = x2ms_adapter.tensor_api.long(new_tensor([0] * batch_size)) == 1
    ...
    for t in range(max_len):
        ...
        all_gen_outputs.append(x2ms_adapter.tensor_api.unsqueeze(gen_output, 0))
        all_decode_outputs.append(decode_outputs)

        probs, ids = model.to_word(data, x2ms_adapter.nn_functional.softmax(gen_output, dim=1), 1, sampling=True)
        ...
        this_end = indice == EOS
        if t == 0:
            x2ms_adapter.tensor_api.masked_fill_(indice, this_end, UNK)
        elif t == max_len - 1:
            indice[:] = EOS
            x2ms_adapter.tensor_api.masked_fill_(indice, end, PAD)
        else:
            x2ms_adapter.tensor_api.masked_fill_(indice, end, PAD)
        indices.append(x2ms_adapter.tensor_api.unsqueeze(indice, 1))
        end = end | this_end

        decoder_input = model.generation_to_decoder_input(data, indice)

    all_gen_outputs = x2ms_adapter.tensor_api.contiguous(x2ms_adapter.tensor_api.transpose(x2ms_adapter.cat(all_gen_outputs, dim=0), 0, 1))

    return x2ms_adapter.cat(indices, dim=1), encode_outputs, init_decoder_states, all_decode_outputs, all_gen_outputs
...
def greedy(model, data, vocab2id, max_len=20, encode_outputs=None, init_decoder_states=None, init_feedback_states=None):
    ...
    all_inputs = x2ms_adapter.tensor_api.unsqueeze(decoder_input, 1)
    all_decode_outputs = [dict({'state': decoder_states})]

    greedy_indices = list()
    greedy_end = x2ms_adapter.tensor_api.long(new_tensor([0] * batch_size)) == 1
    for t in range(max_len):
        if t != 0:
        ...
        if t == 0:
            x2ms_adapter.tensor_api.masked_fill_(greedy_indice, greedy_this_end, UNK)
        else:
            x2ms_adapter.tensor_api.masked_fill_(greedy_indice, greedy_end, PAD)
        greedy_indices.append(x2ms_adapter.tensor_api.unsqueeze(greedy_indice, 1))
        greedy_end = greedy_end | greedy_this_end

        decoder_input = model.generation_to_decoder_input(data, greedy_indice)
        all_inputs = x2ms_adapter.cat([all_inputs, x2ms_adapter.tensor_api.unsqueeze(decoder_input, 1)], dim=-1)  # 所有生成过的词
    # 查看内容
    # print('test content: \n', all_inputs)
    greedy_indice = x2ms_adapter.cat(greedy_indices, dim=1)
...
def beam(model, data, vocab2id, max_len=20, width=5, encode_outputs=None, init_decoder_states=None):
    ...
    for l in range(max_len + 1):
        ...
        for i in range(len(fringe)):
            ...
            for j in range(width):
                loss = -x2ms_adapter.tensor_api.log(math, x2ms_adapter.tensor_api.item(probs[i, j]) + 1e-10)

                n_new = Node(parent=n, state=get_data(i, decode_outputs), word=None, value=x2ms_adapter.tensor_api.item(ids[i, j]), cost=loss,
    ...
    outputs = []
    for i in range(batch_size):
        x2ms_adapter.tensor_api.sort(results[i], key=lambda n: n.cum_cost / n.length)
