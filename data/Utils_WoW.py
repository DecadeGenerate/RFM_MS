...
import random
import x2ms_adapter
nltk.data.path.append("/home/ma-user/modelarts/user-job-dir/RFM-main_x2ms/")
    ...
    with codecs.open(file, encoding='utf-8') as f:
        for line in f:
            ...
            if len(temp[0]) < 1:
                ...
                temp[0] = x2ms_adapter.tensor_api.split(temp[0], ';')
            temp[2] = x2ms_adapter.tensor_api.split(temp[2], ';')
    ...
    with codecs.open(file, encoding='utf-8') as f:
        for line in f:
            ...
            if temp[0] in poolset:
                passage[temp[0]] = x2ms_adapter.tensor_api.split(' [SEP] '.join(
                    [' '.join(nltk.word_tokenize(sent)) for sent in nltk.sent_tokenize(temp[1])]), ' ')  # list的形式
    ...
    if dataset == "wizard_of_wikipedia":
        ...
        with codecs.open(file, encoding='utf-8') as f:
            for line in f:
                ...
                if temp[1] == 'train':
                    x2ms_adapter.tensor_api.add(train, temp[0])
                elif temp[1] == 'dev':
                    x2ms_adapter.tensor_api.add(dev, temp[0])
                elif temp[1] == 'test_seen':
                    x2ms_adapter.tensor_api.add(test_seen, temp[0])
                elif temp[1] == 'test_unseen':
                    x2ms_adapter.tensor_api.add(test_unseen, temp[0])
        ...
        with codecs.open(file, encoding='utf-8') as f:
            for line in f:
                ...
                if temp[1] == 'train':
                    ...
                    x2ms_adapter.tensor_api.add(test, temp[0])
