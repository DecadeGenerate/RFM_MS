...
class SampleCache(object):
    ...
    def put(self, ids, samples):
        ...
        for i in range(len(ids)):
            id = x2ms_adapter.tensor_api.item(ids[i])
    ...
    def get(self, ids):
        ...
        for i in range(len(ids)):
            ...
            sents.append(x2ms_adapter.tensor_api.unsqueeze(self.cache[id][j], 0))
        sents = x2ms_adapter.cat(sents, dim=0)
        return sents
...
def disorder_sampling(batch_instance):
    ...
    for i in range(batch_size):
        ...
        while n == i:
            n = random.randint(0, batch_size - 1)
        results.append(x2ms_adapter.tensor_api.unsqueeze(batch_instance[n], 0))
    return x2ms_adapter.cat(results, dim=0)
...
def build_words_vocab_map(words, vocab2id):
    src_map1 = x2ms_adapter.zeros((len(words), len(vocab2id)), requires_grad=False)
...
def build_vocab_vocab_map(id2vocab1, vocab2id2):
    src_map1 = x2ms_adapter.zeros((len(id2vocab1), len(vocab2id2)), requires_grad=False)
    vocab_overlap = x2ms_adapter.ones(len(id2vocab1), requires_grad=False)
    for id in id2vocab1:
...
def merge1D(sequences, max_len=None, pad_value=None):
    ...
    if pad_value is None:
        padded_seqs = x2ms_adapter.tensor_api.type_as(x2ms_adapter.zeros(len(sequences), max_len, requires_grad=False), sequences[0])
    else:
        padded_seqs = x2ms_adapter.tensor_api.type_as(x2ms_adapter.full((len(sequences), max_len), pad_value, requires_grad=False), sequences[0])
...
def merge2D(sequences, max_len1=None, max_len2=None):
    lengths1 = [x2ms_adapter.tensor_api.x2ms_size(seq, 0) for seq in sequences]
    lengths2 = [x2ms_adapter.tensor_api.x2ms_size(seq, 1) for seq in sequences]
    max_len1 = max(lengths1) if max_len1 is None else max_len1
    max_len2 = max(lengths2) if max_len2 is None else max_len2
    padded_seqs = x2ms_adapter.tensor_api.type_as(x2ms_adapter.zeros((len(sequences), max_len1, max_len2), requires_grad=False), sequences[0])
    for i, seq in enumerate(sequences):
        end1 = lengths1[i]
        end2 = lengths2[i]
    ...
    for key, value in data.items():
        if value is not None:
            if isinstance(value, mindspore.Tensor):
                ones[key] = x2ms_adapter.tensor_api.unsqueeze(value[i], 0)
            elif isinstance(value, dict):
                ones[key] = value[x2ms_adapter.tensor_api.item(data['id'][i])]
            else:
                ones[key] = [value[i]]
        else:
            ones[key] = None
...
def concat_data(datalist):
    ...
    for key in datalist[0]:
        ...
        if value is not None:
            if isinstance(value, mindspore.Tensor):
                data[key] = x2ms_adapter.cat([datalist[i][key] for i in range(size)], dim=0)
            elif isinstance(value, dict):
                data[key] = dict()
                for i in range(size):
                    data[key][x2ms_adapter.tensor_api.item(datalist[i]['id'])] = datalist[i][key]
            else:
...
def load_vocab(vocab_file, t=0):
    ...
    with codecs.open(vocab_file, encoding='utf-8') as f:
        for line in f:
            try:
                name, freq = x2ms_adapter.tensor_api.split(line.strip('\n').strip('\r'), '\t')  # 根据去除首尾的换行符，根据\t分开
            except Exception:
                continue
...
def load_embedding(src_vocab2id, file):
    ...
    with codecs.open(file, encoding='utf-8') as f:
        for line in f:
            splitLine = x2ms_adapter.tensor_api.split(line)
            word = splitLine[0]
            model[word] = x2ms_adapter.x2ms_tensor([float(val) for val in splitLine[1:]])
    matrix = x2ms_adapter.zeros((len(src_vocab2id), 100))
    x2ms_adapter.nn_init.xavier_uniform_(matrix)
    for word in model:
